...
int suspend(char* path, int fd_out, int jobid, int max_jobs)
{
	int i, j = 0;
	int max = max_jobs;
	...
	sprintf(buffer,"suspend %d",jobid);
	...
	{ 
		...
		{ 
			sprintf(buffer,"Job doesnt exist",jobid);
			
			if( (nwrite = write(fd_out,buffer,strlen(buffer)+1)) == -1 ) 
			{ perror("Command suspend: Error writing to console"); }
